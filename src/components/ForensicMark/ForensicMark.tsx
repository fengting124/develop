...
export interface ForensicMarkProps {
  ...
  active?: boolean;
  tooltip?: string;
  /** 'right'(默认): 标签从右下角向右延伸; 'left': 从左下角向左延伸（用于靠右边缘的标记） */
  calloutSide?: 'right' | 'left';
}

export function ForensicMark({ x, y, w, h, label, confidence, active = false, tooltip, calloutSide = 'right' }: ForensicMarkProps) {
  const isLeft = calloutSide === 'left';
  return (
    <motion.div
    ...
    >
      ...
      {/* 引线与标签 */}
      <div className={`${styles.callout} ${isLeft ? styles.calloutLeft : ''}`}>
        <motion.div
        ...
        <motion.div
          ...
          variants={{ hidden: { opacity: 0, x: isLeft ? 10 : -10 }, visible: { opacity: 1, x: 0, transition: { duration: 0.3, delay: 0.7 } } }}
        >
